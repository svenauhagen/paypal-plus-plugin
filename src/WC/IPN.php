...
class IPN {
	...
	protected function payment_status_reversed( $order, $posted ) {
		...
		$this->send_ipn_email_notification(
			...
			sprintf( __( 'Order #%s has been marked on-hold due to a reversal - PayPal reason code: %s',
				'woo-paypal-plus' ), $order->get_order_number(), wc_clean( $posted['reason_code'] ) )
		);
	}
	...
	protected function payment_status_canceled_reversal( $order, $posted ) {
		...
		$this->send_ipn_email_notification(
			sprintf( __( 'Reversal cancelled for order #%s', 'woo-paypal-plus' ), $order->get_order_number() ),
			sprintf( __( 'Order #%s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %s',
				'woo-paypal-plus' ), $order->get_order_number(),
